...
namespace IngameDebugConsole
{
	public class ConsoleMethodInfo
	{
        ...
        /// <summary>If this command not custom (help, sysinfo and etc)</summary>
        public readonly bool isSystemCommand;

		public ConsoleMethodInfo( MethodInfo method, Type[] parameterTypes, object instance, string command, string description, string signature, bool isSystemCommand, string[] parameters )
		{
			this.method = method;
	...
	public static class DebugLogConsole
	{
		...
		static DebugLogConsole()
		{
			...
			AddCommand("help", "Prints all commands", LogAllCommands, isSystemCommand: true);
			AddCommand("sysinfo", "Prints system information", LogSystemInfo, isSystemCommand: true);
		}
		...
		public static void LogAllCommands()
		{
			...
			stringBuilder.Append("Game commands:");

			foreach (ConsoleMethodInfo method in methods.Where(m => m.isSystemCommand == false))
			{
				string signature = method.signature.Replace(method.command, "");
				stringBuilder.Append($"\n- <b>{method.command}</b> {signature}");
			}

			stringBuilder.Append("\n\nSystem commands:");

			foreach (ConsoleMethodInfo method in methods.Where(m => m.isSystemCommand))
			{
				string signature = method.signature.Replace(method.command, "");
				stringBuilder.Append($"\n- <b>{method.command}</b>: {method.description} <color=#bbb><size=12>{method.signature}</size></color>");
			}
		...
		public static void AddCommand( string command, string description, Action method, bool isSystemCommand = false) { AddCommand( command, description, method.Method, method.Target, isSystemCommand); }
		...
		private static void AddCommand( string command, string description, MethodInfo method, object instance = null, bool isSystemCommand = false)
		{
			...
			methods.Insert( commandIndex, new ConsoleMethodInfo( method, parameterTypes, instance, command, description, methodSignature.ToString(), isSystemCommand, parameterSignatures ) );
		}
